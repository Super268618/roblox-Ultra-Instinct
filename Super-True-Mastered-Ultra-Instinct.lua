-- MASTERED ULTRA INSTINCT - FULL POWER MODE
-- Auto-dodge, auto-counter, aura, speed boost, afterimages, and MORE!
...
-- MASTERED UI SETTINGS
local UI_ENABLED = false
local VELOCITY_THRESHOLD = 100
local DODGE_DISTANCE = 25
local SPEED_BOOST = 2.5
local PROXIMITY_RANGE = 15 -- Detect enemies this close
local AFTERIMAGE_INTERVAL = 0.08
...
FXFolder.Name = "MasteredUI_FX"

-- GUI
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "MasteredUIGUI"
...
StatusLabel.Position = UDim2.new(0.1, 0, 0.18, 0)
...
StatsFrame.Position = UDim2.new(0.1, 0, 0.32, 0)
...
ModeLabel.Text = "🌟 Mode: Pure Defense"
...
ToggleButton.Position = UDim2.new(0.1, 0, 0.68, 0)
...
local function createAfterimage()
	...
	for _, part in pairs(Character:GetDescendants()) do
		if part:IsA("BasePart") and part.Transparency < 1 and part.Name ~= "HumanoidRootPart" then
			...
			clone.Color = Color3.fromRGB(200, 220, 255)
...
local function createDodgeEffect(position)
	...
	TweenService:Create(part, TweenInfo.new(0.4), {Size = Vector3.new(12, 12, 12), Transparency = 1}):Play()
...
-- MAIN DODGE SYSTEM
local function startMasteredUI()
	...
	AuraConnection = RunService.Heartbeat:Connect(function()
		...
		aura.Size = Vector3.new(8 * scale, 8 * scale, 8 * scale)
	end)
...
print("⚡🔥 MASTERED ULTRA INSTINCT LOADED 🔥⚡")
print("PURE DODGE MODE: Auto-dodge, Aura, Speed, Afterimages")
print("Like True Mastered UI Goku - PURE DEFENSE!")
print("Click AWAKEN to enter Mastered UI state!")
